...
export const getRepoReleaseData = async (name?: string) => {
  ...
  try {
    const dataRequest = await fetch(
      `https://api.github.com/repos/PrakasRavichandran/${name}/releases/latest`,
      authHeaders,
    );
    ...
    const extraRepoData = {
      success: false,
      url: `https://github.com/PrakasRavichandran/${name}`,
    };
    const defaultDownloadLink = `https://github.com/PrakasRavichandran/${name}/releases/latest/`;
